...
class DataProcess:
    ...
    def refactor_AFSNT(self):
        ...
        self.rm_rows = []
        start_time = dt.datetime(2017, 1, 1, hour=0, minute=0)
        for cur in range(len(self.data)):
            ...
            #STT 상대시간 환산
            cur_time = dt.datetime(self.data[cur][0], self.data[cur][1], self.data[cur][2], hour=self.data[cur][11]//60, minute=self.data[cur][11]%60)
            dif_time = cur_time-start_time
            self.data[cur].append(dif_time.days*24*60*60 + dif_time.seconds//60)
        # STTATT 행 추가
        self.feature_names.append('STTATT')
        # STT 상대시간(분 형태)
        self.feature_names.append('STT_MM')
        # CNL 행 삭제
    ...
    def merge_2(self):
        STT_list = [(d[14], d[8], idx) for idx, d in enumerate(self.data)]
        Dep_list = [(a,i) for a,b,i in STT_list if b==0]
        Arv_list = [(a,i) for a,b,i in STT_list if b==1]
        STT_list.sort()
        for i in range(len(self.data)):
            if self.data[i][8] == 0:
                for i in range()
    ...
    def run(self):
        ...
        print("**********Out************")
        print("*************************")
        self.out("merge_AFSNT_weather_2.csv")
