...
class DataProcess:
    ...
    def refactor_AFSNT(self):
        ...
        for cur in range(len(self.data)):
            ...
            try:
                ...
                self.data[cur][7] = self.data[cur][7][:len(self.data[cur])-1]
            #REG도 숫자로?
            #self.data[cur][8] = char2idx['REG'][self.data[cur][8]]
            #AOD 숫자로?
            self.data[cur][8] = char2idx['AOD'][self.data[cur][8]]
            #IRR?
            #self.data[cur][10] = char2idx['IRR'][self.data[cur][10]]
            #STT 숫자로
            t,m = map(int, self.data[cur][9].split(':'))
            self.data[cur][9] = t*60 + m
            #ATT 숫자로
    ...
    def merge(self):
        data_ydmt = [str(d[0]) + str(d[1]).rjust(2, "0") + str(d[2]).rjust(2, "0") + str(d[9]//60).rjust(2, "0") for d in self.data]
        ydmt = defaultdict()
        def func(i, temp):
            print(i,temp)
            try:
    ...
    def drop(self):
        drop_list = [('SDT_YY', 1), ('DLY',1), ('DLY_RATE',1)]
    ...
    def run(self):
        ...
        print("**********Out************")
        print("*************************")
        self.out("merge_AFSNT_DLY_weather.csv")

if __name__ == "__main__":
    DataProcess('AFSNT_DLY.csv', 'weather_16_30.csv').run()