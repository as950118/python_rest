...
class Data:
    def __init__(self, rfname, len_data=900000, len_col=15):
        ...
        self.len_col = len(data[0])

        # feature(x_data)와 result(y_data) 나누기
        x_data = np.array([list(map(int, [e for e in elem[:9]] + [e for e in elem[10:]] )) for elem in data[:self.len_data]])
        y_data = np.array([int(elem[9]) for elem in data[:self.len_data]])
...
if __name__=="__main__":
    ...
    while 1:
        if os.path.isdir(orgdirname+str(i)):
            ...
            Main("../merge_AFSNT_weather.csv").run()
            break